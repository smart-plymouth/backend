...
def list_cases():
    ...
    Query params:
        ...
        received_date - exact received date (YYYY-MM-DD)
        received_from - start of received date range (inclusive)
        received_to   - end of received date range (inclusive)
    ...
    # Exact received date filter
    if received_date:
        parsed = _parse_date(received_date)
        if parsed is None:
            return jsonify({"error": "Invalid received_date format. Use YYYY-MM-DD."}), 400
        query = query.filter(PlanningCase.received_date == parsed)
    else:
        # Date range filters
        if received_from:
            parsed = _parse_date(received_from)
            if parsed is None:
                return jsonify({"error": "Invalid received_from format. Use YYYY-MM-DD."}), 400
            query = query.filter(PlanningCase.received_date >= parsed)

        if received_to:
            parsed = _parse_date(received_to)
            if parsed is None:
                return jsonify({"error": "Invalid received_to format. Use YYYY-MM-DD."}), 400
            query = query.filter(PlanningCase.received_date <= parsed)
