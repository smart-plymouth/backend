...
class PlanningCase(db.Model):
    ...
    supports = db.relationship(
        "PlanningSupport", back_populates="case", lazy="dynamic"
    )
...
class PlanningSupport(db.Model):
    __tablename__ = "planning_supports"
    ...
    support_reason = db.Column(db.UnicodeText, nullable=False)
    ai_rationalisation = db.Column(db.UnicodeText, nullable=False)
    created_at = db.Column(
        db.DateTime(timezone=True), server_default=db.func.now(), nullable=False
    )

    case = db.relationship("PlanningCase", back_populates="supports")
