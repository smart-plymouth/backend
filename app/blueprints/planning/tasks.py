...
def _run_ai_analysis(metadata, document_texts, reference):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", (
            ...
            "ONLY make claims about things explicitly stated in the application metadata "
            "and documents provided. Do NOT assume or invent facts about the surrounding "
            "area, nearby landmarks, traffic conditions, local amenities, or any other "
            "context that is not directly stated in the information given. Do NOT generate "
            "generic facts about Plymouth or assume the application may affect locations "
            "or facilities not referenced in the provided details. If something is not "
            "mentioned in the application, do not reference it.\n"
        )),
        ("human", (
...
def _generate_objections(metadata, document_texts, reference, analysis):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", (
            ...
            "SEPARATE grounds for objection to a planning application.\n\n"
            "## YOUR PRIMARY TASK:\n"
            "Analyse the specific planning application below and produce ALL legitimate "
            "grounds for objection that could reasonably be raised. Be EXHAUSTIVE — identify "
            "every distinct planning concern that this application engages, no matter how "
            "many or few that turns out to be. A minor extension might only have 1-2 valid "
            "objections. A large mixed-use development could have 10-15+. Let the application "
            "drive the number, not an arbitrary target.\n\n"
            "Each objection must address a DIFFERENT planning concern "
            "specific to THIS application. Do NOT combine multiple concerns into one objection.\n\n"
            "Examples of separate objections (each would be its own entry):\n"
            "- Loss of privacy due to overlooking from proposed first-floor windows\n"
            "- Increased traffic on a narrow residential street with no turning provision\n"
            "- Overdevelopment of the plot resulting in insufficient garden space\n"
            "- Design out of character with the surrounding Victorian terrace\n"
            "- Loss of on-street parking due to removal of existing dropped kerb\n\n"
            "Each objection must be grounded in the SPECIFIC details of this application "
            "(the address, what is proposed, the site context) — not generic planning concerns.\n\n"
            "## GROUNDING RULE (CRITICAL):\n"
            "ONLY make claims about things explicitly stated in the application metadata, "
            "documents, and policy extracts provided. Do NOT assume or invent facts about "
            "the surrounding area, nearby landmarks, traffic conditions, local amenities, "
            "or any other context that is not directly stated in the information given. "
            "Do NOT generate generic facts about Plymouth or assume the application may "
            "affect locations or facilities not referenced in the provided details. "
            "If something is not mentioned in the application, do not reference it.\n\n"
            "## POLICY CITATIONS:\n"
            "Back each objection with specific policy references from the extracts provided:\n\n"
            "For NPPF: cite the paragraph number and chapter name, and briefly state what "
            "it requires. Example: 'NPPF Chapter 12, paragraph 135 requires development to "
            ...
            "'JLP Policy DEV1 (Protecting health and amenity) states that development must "
            "not result in unacceptable impacts on privacy through overlooking.'\n\n"
            ...
            "## JSON OUTPUT FORMAT:\n"
            "Respond with a JSON object containing a single key \"objections\" whose value "
            ...
            "objection for THIS specific application, referencing application details "
            "AND citing the relevant JLP/NPPF policy with paragraph/section numbers\n\n"
            "## WHAT NOT TO DO:\n"
            "- Do NOT combine multiple different concerns into a single objection\n"
            "- Do NOT write generic objections that could apply to any application\n"
            "- Do NOT include objections based on: property values, business competition, "
            "personal disputes, or loss of private views\n"
            "- Do NOT include an objection unless it relates to a legitimate planning matter\n"
        )),
        ("human", (
            "Planning Application Reference: {reference}\n\n"
            "## AI Assessment\n"
            ...
            "Now identify ALL legitimate grounds for objection specific to this application. "
            "Be thorough — consider every planning matter this proposal engages. "
            "Return JSON with key \"objections\" containing the array."
        )),
    ])
